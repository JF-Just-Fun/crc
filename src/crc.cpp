...
uint64_t CRC::singleCRC(uint64_t data) const
{
  uint64_t crc = 0;
  const uint64_t singleMask = 1ULL << (this->params.bitWidth - 1);

  for (int i = 0; i < this->params.bitWidth; ++i)
  {
    bool bit = crc & singleMask;
    crc <<= 1;
    if (data & singleMask)
    {
      bit = !bit;
...
void CRC::generateCrcTable()
{
  ...
  for (uint64_t i = 0; i < range; ++i)
  {
    ...
    if (this->params.refIn)
      this->reverseTable[i] = this->reverseBits(i, 8);
  }
}
...
TransformOut CRC::file(const std::string &filePath)
{
  ...
  while (file.read(buffer, BLOCK_SIZE))
  {
    for (char i : buffer)
    {
      crc = calculateCRC(i, crc);
    }
  }
  const size_t lastBlockSize = file.gcount();
...
std::vector<std::string> CRC::getPoly()
{
  ...
  keys.reserve(CRC::predefinedParams.size());
